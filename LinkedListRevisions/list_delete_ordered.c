...
struct node *find_prev(struct node *head, struct node *check_node);
...
struct node *remove_disorder(struct node *head) {
    ...
    struct node *current = last_node;
    struct node *prev = find_prev(new_head, last_node);

    while (prev != NULL) {
        if (prev->data > current->data) {
            struct node *prev_to_prev = find_prev(new_head, prev);
            if (prev_to_prev == NULL) {
                free(prev);
                new_head = current;
                prev = NULL;
            } else {
                prev_to_prev->next = current;
                free(prev);
                prev = prev_to_prev;
            }
        } else {
            current = prev;
            prev = find_prev(new_head, current);  
        }
